...
namespace Thor
{
	...
	public:
		...
		{
			int32 length = PlatformString::Strlen(source) + 1; //+1 for null terminator.
			m_data.AddUninitialized(length);
			PlatformString::Convert(m_data.Data(), length, source, length);
		}

		FORCEINLINE String& operator=(const CharType* other)
		{
			m_data.Clear();
			int32 length = PlatformString::Strlen(other) + 1; //+1 for null terminator.
			m_data.AddUninitialized(length);
		...
		FORCEINLINE String& operator=(const InCharType* other)
		{
			...
			int32 length = PlatformString::Strlen(other) + 1; //+1 for null terminator.
			m_data.AddUninitialized(length);
			PlatformString::Convert(m_data.Data(), length, other, length);
			return *this;
		}
